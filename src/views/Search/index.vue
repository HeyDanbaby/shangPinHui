  ...
  <div>
    ...
    <div class="main">
      <div class="py-container">
        ...
        <!--selector-->
        <SearchSelector
          @trademarkInfo="ParentTrademarkInfo"
          @attrInfo="ParentAttrInfo"
        />
        ...
        <div class="details clearfix">
          ...
          <div class="goods-list">
            <ul class="yui3-g">
              <li
                class="yui3-u-1-5"
                v-for="(good, index) in pageGoodsList"
                :key="good.id"
              >
                <div class="list-wrap">
                  <div class="p-img">
...
export default {
  ...
  methods: {
    ...
    // 点击面包屑，删除关键字
    removeKeyword() {
      // 置空search中searchParams的关键字
      this.searchParams.keyword = undefined;
      this.getData();
      // header中的输入框里的关键字也要置空 - 兄弟组件间的通信 - main.js里配置$bus全局事件总线
      this.$bus.$emit("clear"); // -header组件-mounted
      // 地址栏路由跳转 params参数不需要了 但是query参数得留着
      // this.$router.push({ name: "search" });
      if (this.$route.query) {
        this.$router.push({ name: "search", query: this.$route.query });
      }
    },
    // 自定义事件 子传父 子组件那里触发事件-父组件接收
    ParentTrademarkInfo(trademark) {
    ...
    // 平台属性
    ParentAttrInfo(attrs, attrValue) {
      // ['属性ID:属性值:属性名']
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      // console.log('监听路由信息是否发生变化，发生变化,再次发请求');
      ...
      this.getData();
      // console.log(this.searchParams);
      // 每一次请求完毕后，要把相应的上一次分类的id置空，让它接收下一次的id
